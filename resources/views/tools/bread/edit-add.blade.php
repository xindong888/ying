...
@section('page_header')
    <div class="page-title">
        ...
        @if (isset($dataType->id))
            {{ __('voyager::bread.edit_bread_for_table', ['table' => __($dataType->name)]) }}
        @else
            ...
            <div class="col-md-12">
                ...
                <!-- CSRF TOKEN -->
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i
                                        class="voyager-bread"></i> {{ ucfirst($table) }} {{ __('voyager::bread.bread_info') }}
                            </h3>
                    ...
                    <div class="panel panel-primary panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i
                                        class="voyager-window-list"></i> {{ __('voyager::bread.edit_rows', ['table' => $table]) }}
                                :</h3>
                            <div class="panel-actions">
                                <a class="panel-action voyager-angle-up" data-toggle="panel-collapse"
                                   aria-hidden="true"></a>
                                    ...
                                    <div class="row row-dd">
                                        ...
                                        <div class="col-xs-2">
                                            ...
                                            @else
                                                <div class="form-group">
                                                    <select name="field_input_type_{{ $data['field'] }}"
                                                        ...
                                                        @foreach (Voyager::formFields() as $formField)
                                                            ...
                                                            @endphp
                                                            <option value="{{ $formField->getCodename() }}" {{ $selected ? 'selected' : '' }} data-lang="_{{$key++}}{{ __('voyager::form.'.$formField->getName()) }}">
                                                                @if(config('app.locale')=='zh_CN')
                                                                    {{ __('voyager::form.'.$formField->getName()) }}
                                                                @else
                                                                    {{ $formField->getName() }}
                                                                @endif
                                                            </option>
                                                ...
                                                <div class="label label-success" data-field="{{ $data['field'] }}"
                                                     style="margin-top:10px;">可选项说明
                                                </div>
                                            @endif
                                        </div>
    ...
    <script>
        ...
        layui.use(['jquery', 'layer'], function () {
            ...
            $('.label-success').click(function () {
                var field = $(this).attr('data-field');//获取字段名称
                var s = $("select[name=field_input_type_" + field + "]").find('option:selected').attr('data-lang')
                layer.open({
                    type:2,
                    area: ['1000px', '800px'],
                    shadeClose:true,
                    content:'https://xjke.com/docs/index.html#/?id='+s.toLowerCase()
                })
